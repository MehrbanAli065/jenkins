                ...
                script {
                    try {
                        withCredentials([file(credentialsId: 'key', variable: 'GCP_KEY_FILE')]) {
                            ...
                            sh 'gcloud config set project pipeline {
    agent any
...
' // Replace with your GCP project ID
                            sh 'gcloud compute scp index.html user@your-gcp-instance:/path/to/destination' // Replace with your GCP instance SSH username, instance name, and destination path
                            echo 'Successfully deployed index.html to Google Cloud server'
                        }
                    } catch (Exception e) {
                        currentBuild.result = 'FAILURE'
